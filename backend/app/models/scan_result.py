...
class ScanResult(TimestampMixin, Base):
    ...
    foreign_net_buy_qty: Mapped[int] = mapped_column(BIGINT, nullable=False)
    foreign_net_buy_confirmed_qty: Mapped[int | None] = mapped_column(BIGINT, nullable=True)
    foreign_net_buy_snapshot_qty: Mapped[int | None] = mapped_column(BIGINT, nullable=True)
